...
public class PlayerSoundManager : MonoBehaviour
{
    ...
    public AudioSource GetAudioSource(AudioClip clip)
    {
        ...
        return audioSources.ContainsKey(clip) ? audioSources[clip] : null;
    }

    // 런타임에 동적으로 들어온 클립을 재생하는 메서드
    public void PlayWeaponSound(AudioClip clip)
    {
        if (clip == null) return;

        // 이미 등록된 클립이면 해당 AudioSource 사용, 아니면 새로 생성
        if (!audioSources.ContainsKey(clip))
        {
            CreateAudioSource(clip);
        }
        ...
        if (audioSources.ContainsKey(clip))
        {
            audioSources[clip].PlayOneShot(clip);
        }
    }

    // 런타임에 동적으로 들어온 클립을 중지하는 메서드
    public void StopWeaponSound(AudioClip clip)
    {
        if (clip == null) return;
        if (audioSources.ContainsKey(clip))
        {
