...
public class WeaponChange : MonoBehaviour
{
    ...
    public void ChangeWeapon(WeaponType Wt, GameObject Weapon)
    {
        if (Wt == WeaponType.closeType)
        {
            ...
            //Í∑ºÏ†ëÎ¨¥Í∏∞??Í≤ΩÏö∞ Î¨¥Í∏∞?êÏÑú Î≤ÑÌäº ?¥Î≤§?∏Î? ?±Î°ù?òÎäî Í≤ÉÏù¥ ?ÑÎãàÍ∏∞Ïóê Í∑ºÏ†ëÍ≥µÍ≤© Î™®ÏÖò???¨Í∏∞???±Î°ù
        ...
        if (Wt == WeaponType.closeType)
        {
            player_CloseWeapon.GetComponent<PlayerScriptOneHand>().WeaponSynchronization();  //?ÑÏû¨ ?°Ï? Î¨¥Í∏∞ ?§Ïãú ?êÏÉâ
        }
        else if (Wt == WeaponType.longType)
        {
            player_LongWeapon.GetComponent<PlayerScriptRifle>().WeaponSynchronization();    //?ÑÏû¨ ?°Ï? Î¨¥Í∏∞ ?§Ïãú ?êÏÉâ
        }
    }
}
