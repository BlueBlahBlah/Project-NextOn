...
public class Space_PlayerManager : MonoBehaviour
{
    ...
    public void ChangeWeapon(WeaponType Wt, GameObject Weapon)
    {
        ...
        if (Wt == WeaponType.closeType)
        {
            try
            {
                player_CloseWeapon.GetComponent<Space_PlayerScriptOneHand>().WeaponSynchronization();  //현재 잡은 무기 다시 탐색
            }
        ...
        else if (Wt == WeaponType.longType)
        {
            try
            {
                player_LongWeapon.GetComponent<Space_PlayerScriptRifle>().WeaponSynchronization();    //현재 잡은 무기 다시 탐색
            }
            catch (NullReferenceException e)
            {
