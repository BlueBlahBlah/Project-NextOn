...
public class Enemy_Golem : Mob
{
        ...
        else    //살아있는경우
        {
            ...
            if (curHealth < health)
            {
                ...
                else
                {
                    ...
                    // 데미지 애니메이션이 끝났는지 체크
                    if (!anim.GetCurrentAnimatorStateInfo(0).IsName("isDamage"))
                    {
    ...
    void Attack()
    {
        if (!isAttack) // 이미 공격 중이 아닌지 확인
        {
            ...
            if (attackType == 0)
            {
                anim.SetTrigger("isAttack01"); // 첫 번째 공격 애니메이션
            }
            else
            {
                anim.SetTrigger("isAttack02"); // 두 번째 공격 애니메이션
            }
    ...
    IEnumerator HandleAttack()
    {
        ...
        SoundManager.instance.PlayEffectSound("골렘공격");
        ...
        anim.SetBool("isAttack01", false); // 공격 애니메이션 비활성화
        anim.SetBool("isAttack02", false); // 공격 애니메이션 비활성화
    }
