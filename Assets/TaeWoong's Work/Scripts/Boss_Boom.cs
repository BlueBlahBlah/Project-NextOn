...
public class Boss_Boom : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        {
            isPlayerInside = true; 
            Debug.Log("[태웅 디버깅] 에너지 폭발 패턴 시작 DMG : " + Damage); 
        ...
        while (true) // 무한 루프
        {
            ...
            foreach (Collider col in hitColliders)
            {
                if (col.CompareTag("Player"))
                {
                    ...
                    Debug.Log("[태웅 디버깅] 보스(에너지 폭발) -> 플레이어 DMG : " + Damage); // 데미지 로그 출력
                }
            }
    ...
    private IEnumerator ActivateAttackArea()
    {
        ...
        // 공격 영역 활성화
        yield return new WaitForSeconds(0.5f); // 데미지 효과 대기 시간 = 이펙트 나오는 시간
        AttackArea.enabled = true; // 공격 영역 활성화
        Debug.Log("[태웅 디버깅] Boom AttackArea 활성화됨"); // 디버깅 로그 추가
        yield return new WaitForSeconds(activeDuration); // 활성화 시간 대기
        AttackArea.enabled = false; // 공격 영역 비활성화
        Debug.Log("[태웅 디버깅] Boom AttackArea 비활성화됨"); // 디버깅 로그 추가
