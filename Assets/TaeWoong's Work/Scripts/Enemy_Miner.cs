...
public class Enemy_Miner : Enemy
{
    ...
    void Start()
    {
        ...
        materi = GetComponentInChildren<MeshRenderer>().material;
    ...
    IEnumerator OnDamage() // 피격 코루틴 생성
   {
        materi.color = Color.red; // 피격 시 색상을 Red 로 설정
   ...
   void OnTriggerEnter(Collider other) 
    ...
    void Attack()
    {
        if (!isAttack) // 이미 공격 중이 아닌지 확인
        {
            ...
            anim.SetBool("isAttack", true); // 첫 번째 공격 애니메이션
            anim.SetBool("isWalk", false); // 이동 애니메이션 중지
    ...
    IEnumerator HandleAttack()
    {
        ...
        yield return new WaitForSeconds(1.5f); // 공격 애니메이션의 길이에 맞춰 대기
        ...
        anim.SetBool("isWalk", true); // 걷기 애니메이션 활성화
    }
    void UpdateHPBar()
    {
        hpBar.rectTransform.localScale = new Vector3((float)curHealth / (float)maxHealth, 1f, 1f);
    ...
    void Die()
    {
        ...
        Debug.Log("[태웅 디버깅] Enemy_Miner 사망"); // 디버깅 로그 추가
